...
//
//      Copyright (c) Dmitry V. Stefankov, 1997-2009. All rights reserved.
...
BOOLEAN    g_fFloodPing         =   FALSE;  // Like BSD flood ping
...

//--------------------------- Banner message ---------------------------------
  _tprintf( TEXT("Windows Sockets PING (32-bit) v1.2,  Copyright (c) 1997,2009 Dmitry Stefankov\n") );
...
//--------------------------- Process comand parameters ----------------------
  ...
  if ( dwArgc == 1)
  {
     ...
     _tprintf( TEXT("  wsping32 [-afnrvwd] [-c count] [-i wait] [-p pattern] [-s packetsize]\n") );
     ...
     _tprintf( TEXT("  -f      Flood ping.\n") );
     _tprintf( TEXT("  -d   Sends a \"Do not Fragment\" flag in the packet. The packet will\n") );
     ...
     _tprintf( TEXT("  Dmitry V. Stefankov (dmstef22@dstef.pp.ru, dmstef@gmail.com)\n") );
